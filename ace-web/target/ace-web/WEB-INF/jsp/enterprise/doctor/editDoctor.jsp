		...
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
					<div class="panel-heading">修改医生资料
					...
					<div class="panel-body">
						<div class="col-md-6">
							<form role="form" action="${ctx}/jydUser/updateJydUserInfo" method="post">
								...
								<div class="form-group">
									<label>真实姓名</label>
									<input class="form-control"  name="realName" value="${jydUser.realName}">
								</div>
								<div class="form-group">
										<label>性别</label>
								...
								<div class="form-group">
									<label>生日</label>
									<input class="form-control " id="timer" name="birthday" value="<fmt:formatDate value="${jydUser.birthday}" pattern="yyyy-MM-dd"/>">
								</div>
								...
								<div class="form-group">
									<label>地址</label>
									<textarea class="form-control" name="address" rows="3" >${jydUser.address}</textarea>
								</div>
								<div class="form-group">
									<label>注册来源</label>
								...
								<div class="form-group">
									...
									<select class="form-control" name="isDaili">
										<option value="Y" <c:if test="${jydUser.isDaili eq 'Y'}">selected</c:if>>是</option>
										<option value="F" <c:if test="${jydUser.isDaili eq 'F'}">selected</c:if>>否</option>
									</select>
								</div>
								<div class="form-group">
									<label>是否审核</label>
									<select class="form-control" name="isIdveri">
										<option value="Y" <c:if test="${jydUser.isIdveri == 'Y'}">selected</c:if>>是</option>
										<option value="F" <c:if test="${jydUser.isIdveri == 'F'}">selected</c:if>>否</option>
									</select>
								</div>
