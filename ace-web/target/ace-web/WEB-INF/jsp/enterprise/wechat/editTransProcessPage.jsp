					...
					<div class="panel-body">
						<div class="col-md-6">
						      <div class="form-group">
										<label>订单详情</label>
										<button href="${ctx}/wechatTransorder/getTransorderDetail?id=${wechatTransProcess.syTransorder.id}" data-toggle="modal" data-target="#myModal" class="btn btn-primary">查询</button>
								</div>
							   ...
							   <%--  <div class="form-group">
								...
								<form role="form" action="${ctx}/wechatTransProcess/updateTransProcess" method="post">
								<div class="form-group">
										<label>目标医院</label>
										<input class="form-control"  name="hospital"  value="${wechatTransProcess.hospital}" <c:if test="${wechatTransProcess.jsStatus eq '1'}">disabled="disabled"</c:if>>
								</div>
								<div class="form-group">
										<label>转运车到达时间</label>
										<input class="form-control"  id="timer" value="<fmt:formatDate value="${wechatTransProcess.arriveTime}" pattern="yyyy-MM-dd HH:mm:ss"/>" <c:if test="${wechatTransProcess.jsStatus eq '1'}">disabled="disabled"</c:if>>
								</div>
								<div class="form-group">
										<label>转运车到达地址</label>
										<textarea class="form-control"   name="arriveAddress"  rows="3" <c:if test="${wechatTransProcess.jsStatus eq '1'}">disabled="disabled"</c:if>>${wechatTransProcess.arriveAddress}</textarea>
								</div>
								<div class="form-group">
										<label>转运车司机</label>
										<input class="form-control" name="driver" value="${wechatTransProcess.driver}" <c:if test="${wechatTransProcess.jsStatus eq '1'}">disabled="disabled"</c:if>>
								</div>
								<div class="form-group">
										<label>司机电话</label>
										<input class="form-control"  name="driverPhone" value="${wechatTransProcess.driverPhone}" <c:if test="${wechatTransProcess.jsStatus eq '1'}">disabled="disabled"</c:if>>
								</div>
								
								<div class="form-group">
									<label>医院联系人</label>
									<input class="form-control"   name="hoscontactMan" value="${wechatTransProcess.hoscontactMan}" <c:if test="${wechatTransProcess.jsStatus eq '1'}">disabled="disabled"</c:if>>
								</div>
								<div class="form-group">
									<label>医院联系电话 </label>
									<input class="form-control"    name="hoscontactPhone" value="${wechatTransProcess.hoscontactPhone}"<c:if test="${wechatTransProcess.jsStatus eq '1'}">disabled="disabled"</c:if>>
								</div>
								<div class="form-group">
									<label>转运公里数（KM）</label>
									<input class="form-control"   id="distance" name="distance"    onchange="sumDistance()" value="${wechatTransProcess.distance}" <c:if test="${wechatTransProcess.jsStatus eq '1'}">disabled="disabled"</c:if>/>
								</div>
								<div class="form-group">
									<label>误差公里数（KM）</label>
									<input class="form-control"  id="gapdistance" name="gapdistance"  onchange="sumDistance()"  value="${wechatTransProcess.gapdistance}" <c:if test="${syTransProcess.jsStatus eq '1'}">disabled="disabled"</c:if>/>
								</div>
								<div class="form-group">
									<label>总共公里数（KM）</label>
									<input class="form-control"  id="totaldistance" name="totaldistance" readonly="readonly" value="${wechatTransProcess.totaldistance}" >
								</div>
								<div class="form-group">
									<label>每公里价格</label>
								...
								<div class="form-group">
									<label>其他价格</label>
									<input class="form-control"  id="otherPirce" name="otherPrice" onchange="sumPrice()" value="${wechatTransProcess.otherPrice}" <c:if test="${wechatTransProcess.jsStatus eq '1'}">disabled="disabled"</c:if>>
								</div>
								<div class="form-group">
									<label>总共价格</label>
									<input class="form-control"  id="totalPrice" name="totalPrice" readonly="readonly"  value="${wechatTransProcess.totalPrice}" readonly="readonly">
								</div>
								<%-- <div class="form-group">
									<label>折扣</label>
								...
								<div class="form-group">
									...
									<select class="form-control" name="jsStatus" disabled="disabled">
										<option value="1" <c:if test="${wechatTransProcess.jsStatus eq '1'}">selected</c:if>>已结算</option>
										<option value="0" <c:if test="${wechatTransProcess.jsStatus eq '0'}">selected</c:if>>未结算</option>
									</select>
								</div>
								<div class="form-group">
										<label>审核人</label>
										<input class="form-control"   value="${wechatTransProcess.veritor.realName}" disabled="disabled">
								</div>
								<div class="form-group">
										<label>审核时间</label>
										<input class="form-control"   value="<fmt:formatDate value="${wechatTransProcess.veriTime}" pattern="yyyy-MM-dd HH:mm:ss"/>" disabled="disabled">
								</div>
								
							    <input class="form-control"  type="hidden" name="id" value="${wechatTransProcess.id}">
								<c:if test="${wechatTransProcess.jsStatus eq '0'}">
